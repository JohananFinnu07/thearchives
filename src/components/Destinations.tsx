...
const Destinations = () => {
  ...
  // 🔥 AUTO SCROLL FROM SEARCH
  useEffect(() => {
    if (location.state?.scrollTo && scrollContainerRef.current) {
      const scrollTarget = location.state.scrollTo.toLowerCase();

      const element = document.querySelector(
        `[data-search-id="${scrollTarget}"]`,
      ) as HTMLElement | null;
    ...
    >
      ...
      <div className="relative">
        ...
        <motion.button
          ...
          onClick={() => scroll("left")}
          className="absolute left-4 sm:left-8 top-1/2 -translate-y-1/2 z-10 w-12 h-12 rounded-full bg-background/90 backdrop-blur-sm border border-border shadow-elevated flex items-center justify-center text-foreground hover:bg-background transition"
        ...
        <motion.button
          ...
          className="absolute right-4 sm:right-8 top-1/2 -translate-y-1/2 z-10 w-12 h-12 rounded-full bg-background/90 backdrop-blur-sm border border-border shadow-elevated flex items-center justify-center text-foreground hover:bg-background transition"
        ...
        >
          {destinations.map((destination, index) => (
            <motion.div
              key={destination.id}
              data-search-id={destination.name.toLowerCase()}
